...
export async function restoreAttempt(
  ...
  if (!data || data.actorId !== actorId)
    throw new BackendError("INVALID_STATE", "Start a new Shopify connection.");
  trace("H", "PASS", { businessId: data.businessId });
  trace("J", "PASS", { businessId: data.businessId });
  trace("I", Date.parse(data.expiresAt) > Date.now() ? "PASS" : "FAIL", {
    businessId: data.businessId,
    ...(Date.parse(data.expiresAt) > Date.now() ? {} : { code: "EXPIRED" }),
  });
  ...
  trace("M", "PASS", { businessId: context.businessId });
  trace("N", "PASS", { businessId: context.businessId });
