-- Read-only verification after migration 005; run in the intended SQL Editor as postgres.
...
from (values ('public.create_business(uuid,text,text)'),('public.select_business(uuid,uuid)'),('public.shopify_operation(text,uuid,uuid,uuid,jsonb)'),('public.backend_readiness()')) r(routine);
-- Business RPCs: false,true,false. Shopify/readiness RPCs: false,false,true.
